...
# Generate certificate for a domain (ECDSA recommended)
fastcert --ecdsa example.com

# Generate certificate for multiple domains and IPs
fastcert --ecdsa example.com localhost 127.0.0.1 ::1

# Generate wildcard certificate
fastcert --ecdsa "*.example.com"
```

**Important:** Always use the `--ecdsa` flag when generating certificates. RSA key generation is not supported in the current version.
...
**ECDSA is the only supported key type** in the current version:
```bash
fastcert --ecdsa example.com
```

ECDSA keys provide the same security as RSA with smaller key sizes (256 bits vs 2048 bits), resulting in:
...
- `--ecdsa` - **Required**: Use ECDSA key (only supported key type)
...
**ECDSA (required --ecdsa flag):**
- Curve: P-256 (secp256r1)
- Key size: 256 bits
- Smaller certificates
- Faster operations
- Provides equivalent security to RSA-2048
- Modern and recommended

**Note:** RSA key generation is not currently supported. Use `--ecdsa` for all certificate generation.
...
3. Verify the certificate includes the domain you're accessing:
   ...
   - Example: `fastcert --ecdsa myapp.dev localhost 127.0.0.1 ::1`
...
### Why must I use --ecdsa?

The current version only supports ECDSA key generation due to limitations in the underlying cryptographic library (rcgen 0.12). RSA support may be added in a future version.

**ECDSA benefits:**
- Smaller certificates (256-bit keys vs 2048-bit RSA)
- Faster cryptographic operations
- Same security level as RSA-2048
- Fully supported by all modern browsers and applications
